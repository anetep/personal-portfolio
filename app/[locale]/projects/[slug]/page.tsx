    ...
    <main className="min-h-screen px-3 py-3 sm:px-5 sm:py-5">
      <article className="mx-auto max-w-[1520px] overflow-hidden rounded-[26px] border border-[var(--line)] bg-[var(--paper)] shadow-[0_12px_35px_rgb(91_62_43_/_10%)]">
        ...
        <div className="mx-auto max-w-[1280px] px-6 md:px-10">
          ...
          <section className={`grid gap-8 border-b border-[var(--line)] py-10 md:py-12 ${project.screenshots.length ? "md:grid-cols-2" : ""}`}><article><h2 className="font-serif text-3xl text-[var(--foreground)]">{project.outcome.title}</h2><p className="mt-5 max-w-3xl text-sm leading-7 text-[var(--muted)]">{project.outcome.description}</p>{"outcomeLinks" in project && <p className="mt-4 flex flex-wrap gap-x-5 gap-y-2 font-mono text-[10px] leading-5">{project.outcomeLinks.map((link) => <a key={link.href} href={link.href} target="_blank" rel="noreferrer" className="border-b border-[var(--line)] text-[var(--muted)] transition-colors hover:border-[var(--coral)] hover:text-[var(--coral)]">{link.label} ↗</a>)}</p>}</article>{project.screenshots.length > 0 && <div><p className="font-mono text-[10px] text-[var(--coral)]">screenshots</p><div className="mt-4 grid gap-3 sm:grid-cols-3">{project.screenshots.map((screenshot) => <div key={screenshot.title} className="border-t border-[var(--line)] pt-3"><h3 className="font-serif text-lg text-[var(--foreground)]">{screenshot.title}</h3><p className="mt-2 text-[10px] leading-4 text-[var(--muted)]">{screenshot.description}</p></div>)}</div></div>}</section>
